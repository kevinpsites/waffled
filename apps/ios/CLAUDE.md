...
### Simulators — a clean set per task, deleted when the task is done

**Create your own simulators for each task; don't install onto a shared one.** Other sessions
reinstall the app on the same named devices, so a shared simulator can quietly run someone
else's build. Create fresh ones (`xcrun simctl create "iPhone 17 (<task>)" <device-type>
<runtime>`) and target them by `id=`. When the task is done, **delete them** —
`xcrun simctl shutdown <id>; xcrun simctl delete <id>` removes the install and all its data,
so finished tasks don't leave gigabytes of simulators behind.
