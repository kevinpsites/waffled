...
@Suite struct RecurrenceBuildTests {
    ...
    @Test func customBuilder() {
        ...
        #expect(Recurrence.buildRrule(st { $0.freq = .custom; $0.unit = .month; $0.interval = 2; $0.monthlyMode = .dayOfMonth }, start: TUE_2ND, cal) == "FREQ=MONTHLY;INTERVAL=2")
        // Explicit ordinal (any week), not just the start's own nth — e.g. "first Tuesday".
        #expect(Recurrence.buildRrule(st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = 1 }, start: TUE_2ND, cal) == "FREQ=MONTHLY;BYDAY=1TU")
        #expect(Recurrence.buildRrule(st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = 2 }, start: TUE_2ND, cal) == "FREQ=MONTHLY;BYDAY=2TU")
        #expect(Recurrence.buildRrule(st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = -1 }, start: TUE_2ND, cal) == "FREQ=MONTHLY;BYDAY=-1TU")
        // Ordinal 0 falls back to the start date's own nth (2026-06-09 is the 2nd Tuesday).
        #expect(Recurrence.buildRrule(st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = 0 }, start: TUE_2ND, cal) == "FREQ=MONTHLY;BYDAY=2TU")
...
@Suite struct RecurrenceParseTests {
    ...
    @Test func intervalRules() {
        ...
        #expect(Recurrence.parseRepeat("FREQ=MONTHLY;INTERVAL=2") == st { $0.freq = .custom; $0.unit = .month; $0.interval = 2; $0.monthlyMode = .dayOfMonth })
        #expect(Recurrence.parseRepeat("FREQ=MONTHLY;BYDAY=1TH") == st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = 1 })
        #expect(Recurrence.parseRepeat("FREQ=MONTHLY;BYDAY=2TU") == st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = 2 })
        #expect(Recurrence.parseRepeat("FREQ=MONTHLY;BYDAY=-1TU") == st { $0.freq = .custom; $0.unit = .month; $0.interval = 1; $0.monthlyMode = .nthWeekday; $0.monthlyOrdinal = -1 })
...
@Suite struct RecurrenceDescribeTests {
    @Test func plainEnglish() {
        ...
        #expect(Recurrence.describeRrule("FREQ=MONTHLY;BYDAY=1TH", start: MON, cal) == "Every month on the first Thursday")
