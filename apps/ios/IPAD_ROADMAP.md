...
- [ ] Recurrence **creation + edit scope** on iOS — no repeat picker in `EventEditSheet`
      (creation web-only) and no this / this-and-following / all scope chooser on edit/delete
      (always whole-series). Server already takes `rrule` on create + `scope`/`occurrenceStart`
      on PATCH/DELETE; pure client work + a Swift port of web's `recurrence.ts` rrule builder.
