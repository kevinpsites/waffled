# Waffled iPad — Project Roadmap

A living checklist for bringing Waffled to iPad as a **full, interactive, web-like native
...
> (**screensaver**) shipped. **Phase 4 branding shipped** — the **Waffled** app icon + a
> cold-launch **bouncing-logo splash** on the cream canvas (product renamed Waffled → Waffled
...
Both ship in **one universal app** — same bundle ID (`app.waffled`), one App Store
...
3. **Reuse the existing iOS data layer + feature views.** `SyncManager` (PowerSync),
   `WaffledAPI`, `Theme.swift` tokens — and the existing phone feature views (`CalendarView`,
...
- [x] Idiom helper: `DeviceExperience` (`App/DeviceExperience.swift`) — `.planner` (iPhone)
      vs `.kiosk` (iPad).
- [x] Entry fork: `RootView` in `WaffledApp.swift` → `AppRoot` (iPhone) vs `KioskRoot` (iPad).
...
- [x] **Nav rail** — `KioskShell` (`Features/Kiosk/KioskShell.swift`): a fixed, always-visible
      ...
      rather than a collapsible `NavigationSplitView`. `WAFFLED_KIOSK_PAGE` launch hook drives
...
- [ ] Then progressively **web-ify each page** for the iPad (multi-column where the web is,
  ...
  - [x] **Calendar** — `KioskCalendarView` (done, accepted): Month (grid + side day panel),
        ...
        is two-column. Test hooks: `WAFFLED_CAL_MODE`, `WAFFLED_KIOSK_OPEN_EVENT/EDIT`.
...
- [x] Photo selection honors the config (source all/favorites/album, per-photo interval,
      shuffle) via `WaffledAPI.screensaverPhotos`. Decoded-image cache + prefetch keep the
...
- [x] **Device identity layer** — `KioskDevice.swift`: long-lived `deviceSecret` in the
      ...
      `WaffledAPI`. Separate from the per-person `AuthTokens` session.
- [x] **`WaffledAPI` kiosk calls** — `pairDevice` (code), `promoteDevice` (admin one-tap),
...
- Data layer: `SyncManager` (PowerSync), `WaffledAPI`, `DashboardModel`, `Theme.swift` tokens.
