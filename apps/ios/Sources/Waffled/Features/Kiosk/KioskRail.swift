...
///
/// Storage is **per device** via `@AppStorage("waffled.kioskRailItems")` — a comma-joined
/// list of `KioskNav` rawValues — matching the other per-display prefs in
/// `KioskDashboard` (`waffled.kioskDashLayout`, `waffled.kioskGoalId`). Because it's a plain
...
enum KioskRail {
    ...
    static let storageKey = "waffled.kioskRailItems"
