...
final class ReviewEventsModel {
    private(set) var recap: [NookAPI.GoalRecapItem] = []
    private(set) var suggestions: [NookAPI.GoalSuggestionItem] = []
    ...
    func draft(for it: NookAPI.GoalRecapItem) -> Draft {
        drafts[it.id] ?? Draft(amount: it.suggestedAmount, people: it.defaultPersonIds)
    }

    func setAmount(_ it: NookAPI.GoalRecapItem, _ amount: Double) {
        var d = draft(for: it); d.amount = max(0, amount); drafts[it.id] = d
    }
    func setPeople(_ it: NookAPI.GoalRecapItem, _ people: [String]) {
        var d = draft(for: it); d.people = people; drafts[it.id] = d
    }

    func confirm(_ it: NookAPI.GoalRecapItem, _ sync: SyncManager) async {
    ...
    func skip(_ it: NookAPI.GoalRecapItem, _ sync: SyncManager) async {
        guard !busy.contains(it.id) else { return }
        busy.insert(it.id); defer { busy.remove(it.id) }
        do {
    ...
    func link(_ s: NookAPI.GoalSuggestionItem, _ sync: SyncManager) async {
    ...
    func dismiss(_ s: NookAPI.GoalSuggestionItem, _ sync: SyncManager) async {
        guard !busy.contains(s.id) else { return }
        busy.insert(s.id); defer { busy.remove(s.id) }
        do {
...
struct ReviewEventsView: View {
    ...
    @State private var editingPeople: NookAPI.GoalRecapItem?
    ...
    private func recapCard(_ it: NookAPI.GoalRecapItem) -> some View {
    ...
    private func canConfirm(_ it: NookAPI.GoalRecapItem, _ d: ReviewEventsModel.Draft) -> Bool {
    ...
    private func suggestionCard(_ s: NookAPI.GoalSuggestionItem) -> some View {
    ...
    @ViewBuilder private func goalChip(_ it: NookAPI.GoalRecapItem) -> some View {
    ...
    private func stepper(_ it: NookAPI.GoalRecapItem, amount: Double) -> some View {
    ...
    private func peopleLabel(_ it: NookAPI.GoalRecapItem, _ people: [String]) -> some View {
...
private struct ReviewPeopleSheet: View {
    ...
    let item: NookAPI.GoalRecapItem
    let members: [SyncedMember]
    @State private var picked: Set<String>
    let onSave: ([String]) -> Void

    init(item: NookAPI.GoalRecapItem, members: [SyncedMember], selected: [String], onSave: @escaping ([String]) -> Void) {
