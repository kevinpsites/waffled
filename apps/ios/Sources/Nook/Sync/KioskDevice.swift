...
enum KioskDeviceStore {
    private static let secretKey = "nook.kiosk.deviceSecret"
    private static let labelKey = "nook.kiosk.deviceLabel"
...
actor KioskDeviceAuth {
    ...
    func refresh() async throws -> String {
        ...
        guard (resp as? HTTPURLResponse)?.statusCode == 200 else {
            ...
            throw NookAPI.APIError.http((resp as? HTTPURLResponse)?.statusCode ?? -1,
                                        String(data: data, encoding: .utf8) ?? "")
        }
        let t = try NookAPI.decoder.decode(TokenResp.self, from: data).accessToken
        cached = t
        return t
    }
