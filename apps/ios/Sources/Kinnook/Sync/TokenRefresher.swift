...
/// in the Keychain; on failure the session is cleared and `.nookAuthExpired` fires so
...
actor TokenRefresher {
    ...
    private func performRefresh() async -> Bool {
        ...
        if code == 401 {
            ...
            AuthTokens.clear()
            await MainActor.run { NotificationCenter.default.post(name: .nookAuthExpired, object: nil) }
            return false
        }
