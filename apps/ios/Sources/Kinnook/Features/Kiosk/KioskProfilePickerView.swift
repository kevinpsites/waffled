...
struct KioskProfilePickerView: View {
    ...
    @State private var profiles: [NookAPI.KioskProfile] = []
    @State private var deviceLabel: String?
    @State private var loaded = false
    @State private var loadError: String?

    // Claim state
    @State private var pinFor: NookAPI.KioskProfile?     // non-nil → PIN pad up
    @State private var claiming: NookAPI.KioskProfile?   // non-nil → spinner on that card
    ...
    private func card(_ p: NookAPI.KioskProfile) -> some View {
        Button {
            claimError = nil
    ...
    private func load(silent: Bool = false) async {
        do {
            ...
            loadError = nil
        } catch let NookAPI.APIError.http(code, _) where code == 401 {
    ...
    private func attempt(_ p: NookAPI.KioskProfile, pin: String?) async -> KioskMode.ClaimOutcome {
        claiming = p
        defer { claiming = nil }
...
struct KioskPinPad: View {
    let profile: NookAPI.KioskProfile
