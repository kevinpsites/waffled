...
struct CookConfirmSheet: View {
    ...
    let title: String
    let matches: [NookAPI.RecipeMatch]
    ...
    init(title: String, matches: [NookAPI.RecipeMatch], onApplied: @escaping (Int) -> Void = { _ in }) {
        self.title = title
    ...
    private func row(_ m: NookAPI.RecipeMatch) -> some View {
    ...
    private func amountLabel(_ m: NookAPI.RecipeMatch) -> String {
    ...
    private func confirm() {
        ...
        busy = true
        Task {
            if !items.isEmpty { _ = try? await NookAPI().pantryConsume(items) }
