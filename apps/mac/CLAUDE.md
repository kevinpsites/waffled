...
- **Never auto-restart in a loop.** The runtime supervises its own children. Auto-start fires
  ...
  that is a person clicking `Start Waffled`.
...
- An `LSUIElement` app has no window, so there is nothing to present a sheet or
  `.confirmationDialog` from. Modal questions are `NSAlert` after
