// The plate — the left column of the Meal Builder. Dishes are grouped by `role`; each group
// ends in a drop zone that doubles as a tap target. See docs/product/meal-builder-plan.md.
import { useState } from 'react'
import type { MealDish, Person } from '../../lib/api'

// Roles are free text server-side (soft scaffolding, not an enum) — these three are the ones
// the builder scaffolds today, in plate order.
...
// What a drag hands to a drop zone. A saved meal FLATTENS on drop — its dishes come in as
// individual rows keeping their own roles. A `dish` is already ON the plate: dropping it
// re-roles it in place rather than adding it again, because meal_recipes is keyed on
// (meal_id, recipe_id) so a recipe can only appear once per plate.
...
// The one rule that is easy to get wrong: `onHand` is null when the pantry module is off, and
// that means "we can't say" — render no on-hand claim at all. It is NOT `have: 0`, and we never
// render "0 of N".
function OnHand({ dish, open, onToggle }: { dish: MealDish; open: boolean; onToggle: () => void }) {
  if (dish.toBuy === 0) {
    // Only a pantry that actually reported can claim everything is on hand.
...
function DishRow({
  ...
  dish: MealDish
  persons: Person[]
  // Absent when there is nowhere to open a dish TO — a row that looks tappable and does
  // nothing is worse than one that doesn't invite the tap.
  onOpen?: () => void
  onRemove: () => void
  ...
  return (
    <div className="mb-dish" draggable onDragStart={onDragDish}>
      {/* A div, not a <button>: the to-buy count inside it is itself a button, and interactive
          content may not nest inside a button. */}
      <div
        {...(onOpen
          ? {
              role: 'button',
              tabIndex: 0,
              // Named explicitly, else the accessible name is the row's whole text — which
              // swallows the nested to-buy control and makes both unaddressable.
  ...
  return (
    <div className="mb-plate">
      {PLATE_ROLES.map((role) => {
        ...
        return (
              // The whole group is the drop target, not just its trailing slot — a populated
              // group's slot sits below its rows, so requiring a hit on it would make
              // Sides → Main a game of pixel-hunting.
          <section
            className="mb-group"
            key={role.key}
            data-role={role.key}
            onDragOver={(e) => {
              e.preventDefault()
              // Answer the drag in its own terms: a browser refuses a drop whose dropEffect
              // contradicts the source's effectAllowed, and refuses it silently by never
              // firing `drop`. Library rows drag as 'copy', plate rows as 'move', so
            ...
            onDragLeave={(e) => {
              // Moving between children fires dragleave on the parent; ignore it unless the
              // pointer actually left the group.
          ...
          >
            ...
            {rows.map((d) => (
              <DishRow
                ...
                onDragDish={(e) => {
                  // A bespoke mime type, never text/plain — a text payload gets pasted into
                  // whatever input the drag happens to end over.
