 ...
 *
 * Regenerated from LVGL's own v9.2.2 lv_conf_template.h (fetched from
 * lvgl/lvgl, not hand-patched from the old v8 file — v9's structure changed
 * too much for that). Two deltas from stock: the Montserrat 12/14/16/24/32
 * fonts enabled below, and the tick source, which is NO LONGER configured
 * here — v9 dropped LV_TICK_CUSTOM/LV_TICK_CUSTOM_INCLUDE entirely in favor
 * of a runtime lv_tick_set_cb(wb_tick_ms) call in main.cpp's setup()
 * (wb_tick_hal.h/.cpp are unchanged; only how they're wired in changed).
 */
...
#define LV_FONT_MONTSERRAT_24 1  // headline size for a 7" kid-facing display
#define LV_FONT_MONTSERRAT_26 0
#define LV_FONT_MONTSERRAT_28 0
#define LV_FONT_MONTSERRAT_30 0
#define LV_FONT_MONTSERRAT_32 1  // "Hi, {name}!" — needs to read from across a room
