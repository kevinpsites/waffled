-- Up Migration
-- Series-level Waffled metadata for a GOOGLE-sourced recurring event. Google sync uses
-- singleEvents=true, so each instance arrives as its own events row, all sharing one
-- ical_uid (each with its own google_event_id). Waffled-owned links (goal_id/goal_step_id)
...
-- Waffled-only — never pushed to Google.
