...
const INTENT_SCHEMA = {
  ...
  properties: {
    ...
    memberType: { type: ['string', 'null'], enum: ['adult', 'caregiver', 'guest', 'teen', 'kid', null], description: 'For kind=person: adult | caregiver | guest | teen | kid (default adult)' },
...
function systemPrompt(ctx: CaptureContext): string {
  ...
  return [
    ...
    '- person: "add my son/daughter/husband/wife/mom/dad/… <name>", "add a family member <name>", "create a profile for <name>" → kind "person". Set "name" (just the person\'s name). Infer "memberType": son/daughter/kid/child → "kid", teen/teenager → "teen", babysitter/nanny/caregiver → "caregiver", guest/visitor → "guest", spouse/husband/wife/partner/mom/dad/parent/adult → "adult"; default "adult" for a bare name. Optionally set "avatarEmoji" (a fitting face), "birthday" (YYYY-MM-DD) ONLY if an actual date is given, and "isAdmin" true only for a clear parent/guardian whose memberType is "adult". NEVER invent a birthday from an age ("age 8" → leave birthday null).',
...
export function finalizeIntent(raw: unknown, ctx: CaptureContext): CaptureIntent {
  ...
  if (kind === 'person') {
    ...
    const memberType = (['adult', 'caregiver', 'guest', 'teen', 'kid'] as const).find((t) => t === mt) ?? 'adult'
    ...
    const isAdmin = memberType === 'adult' && (r.isAdmin == null ? false : !!r.isAdmin)
