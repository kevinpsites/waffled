...
function registerProviderConnect(api: Api, name: CalendarProviderName): void {
  ...
  api.get(`/auth/${name}/calendar/callback`, async (req: Request, res: Response) => {
    ...
    if (q.error) {
      return htmlResult(res, 400, 'Connection cancelled', `${label} Calendar access was not granted. No connection was saved.`)
    }
    ...
    try {
      ...
      return htmlResult(res, 502, 'Connection failed', `Could not complete the ${label} connection. Please try again.`)
    }
...
export function registerCalendarRoutes(api: Api): void {
  ...
  api.get('/api/calendar/google/status', adminRoute(async (tenant) => {
    ...
    return {
      ...
      // ICS feed subscriptions — the Calendars panel lists these below the
      // OAuth accounts. They need no provider config, so they always show.
