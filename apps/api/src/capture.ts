...
const INTENT_SCHEMA = {
  ...
  properties: {
    kind: { type: 'string', enum: ['event', 'task', 'grocery', 'meal'] },
    ...
    quantity: { type: ['string', 'null'], description: 'Grocery amount, e.g. "2 lbs"' },
...
function systemPrompt(ctx: CaptureContext): string {
  ...
  return [
    ...
    `Family members: ${fam}.`,
    '',
    'Kinds: "event" = happens at a date/time; "task" = a chore someone does, maybe recurring; "grocery" = an item to buy; "meal" = a dish to put on the weekly meal plan.',
    ...
    '- grocery: "quantity" is just the amount (e.g. "2 lbs"), or null. Never prefix it with a label.',
...
export function finalizeIntent(raw: unknown, ctx: CaptureContext): CaptureIntent {
  ...
  const kind: CaptureIntent['kind'] =
    kindRaw === 'event' ? 'event' : kindRaw === 'grocery' ? 'grocery' : kindRaw === 'meal' ? 'meal' : 'task'
