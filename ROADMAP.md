...
- [ ] 6.7 Notifications: ride Google reminders + APNs nudges. **Kiosk notifications (local-only, buildable now)** — an in-kiosk "due soon" surface so reminders work without APNs/web-push: a `reminders` table + endpoint, a kiosk banner/toast when a reminder time passes (fires while the kiosk is open), seeded by the calendar's **"Remind me"** (today an AI-suggested nudge only — see 5.7). A later step adds web-push (service worker + VAPID + scheduler) so reminders fire when the kiosk is closed
  - [ ] 6.7-ios **iOS local notifications (event reminders)** — on-device `UNUserNotificationCenter` reminders driven entirely from the local PowerSync `events` mirror, so they fire even when the app is closed/offline. No server/APNs/Apple key (the iOS analog of the kiosk-local reminders above). A `NotificationScheduler` (@MainActor) reconciles a rolling horizon against upcoming events on each events-watch update / foreground / pre-suspend; iOS caps pending local notifications at **64** so it schedules ~7 days/~50 events and `log`s any truncation. Stable `evt:<id>:T-<lead>` identifiers so re-syncs replace (not duplicate); edit/delete → cancel + reschedule. `UNCalendarNotificationTrigger` at `starts_at − leadTime`, all-day → a configurable morning hour in the household tz; tap deep-links to event detail. Per-user Settings (UserDefaults v1): master toggle, lead time (default **15 min**), all-day time (default **8:00**), scope (default **"my events only"**). Self-contained, **zero backend changes**. *(buildable now)*
    ...
    - [ ] 6.7.2 **Snooze + quick actions** — `UNNotificationCategory` actions (Snooze 10m / Done): snooze reschedules a one-off, Done acks the event or completes the chore inline
