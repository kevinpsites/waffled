...
- [x] W2c-goals Goals screen real + interactive: goal cards with progress bars (pooled `shared_total` / per-participant `each_tracks`), create / log / delete (GoalModal · LogModal). *(Verified live with Playwright: create → log 120/1000 → bar fills.)* Milestones/rewards, leaderboards, per-person filter still to come
...
- [~] 6.4 Goals + rewards: real api (`goals` migration 0010 — goal_lists/goals/goal_participants/goal_logs; count/total/habit/checklist types; shared_total vs each_tracks tracking; append-only logs → derived progress) + kiosk Goals screen (cards, create, **log progress**, delete). Still to come: milestones/rewards (reuse stars ledger), leaderboards, parent-approval redemption, per-person privacy (goal_lists/members), habit/checklist UIs, family-challenge hero
