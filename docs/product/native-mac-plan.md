...
In progress. The app exists, carries its own runtime, drives a real server from it, walks a
household through its first run and now updates itself; CI assembles the `.app` and boots it
on every change. What is left is the paperwork: Developer ID signing, notarization and the
DMG (item 5), which the user deliberately deferred — so the app is ad-hoc signed and
Gatekeeper refuses it on any Mac but the one that built it, and the appcast it checks has
nothing to serve until that lands.
...
5. Signing + notarization pipeline (every embedded binary), DMG build, Sparkle appcast.
   `Sparkle.framework` is now part of that: its nested `Autoupdate`, `Updater.app` and XPC
   services are signed **inside-out**, before the app. The appcast itself is generated by
   `apps/mac/Scripts/make-appcast.sh` (item 6) and uploaded beside the DMG. Two things item 6
   found that this item has to finish: the app must be signed **after** the runtime is
   embedded (`build-app.sh` now does the ad-hoc version of that), and **`./waffled release`
   does not bump `apps/mac/project.yml`** — it bumps api, web, compose and iOS — so a
   release whose Mac version was not moved by hand is one Sparkle cannot tell from the last.
6. Updater: the Sparkle appcast, swapping `Waffled.app` and relaunching. The **data** half —
   ...
   *(done — PR #199)* → Sparkle 2 via SPM, pinned exactly, with the EdDSA public key, the
   ...
   - **Known edge, still open:** dragging a newer DMG over a running install has the same
     ...
     say so in the DMG's README or the app will detect it (its own bundle version against
     `bundle.version` in `status --json`).
   - `WAFFLED_APPCAST_URL` overrides the feed for a test run — a seam, not a boundary; the
     ...
     of releases into the feed for item 5's release script.
