...
attendees; **kids are Waffled-only participants** (no email, not sent to Google). `events.person_id`
...
- **Calendar:** Google authoritative for **google-origin** events on the *Google-owned* columns
  (inbound upsert touches only those); **Waffled authoritative** for Waffled-owned columns
...
| Participants | `event_participants` table; kids are Waffled-only (no Google attendee) |
