...
Nook recipes can be written in a small Markdown format. You can paste a recipe in
this format directly into the app — **Recipes → ＋ New recipe → 📋 Paste markdown** —
and Nook parses it into a structured recipe (title, metadata, ingredients grouped by
...
ChatGPT/Claude, ask it to fill in a dish, and paste the result back into Nook.
