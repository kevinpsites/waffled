...
- [x] 3.4 **Role-based permissions** (DONE 2026-06-24). `member_type` (adult/teen/kid) now carries real authorization instead of the binary `is_admin` gate. Four capabilities — `chore.manage`, `chore.approve`, `reward.manage`, `reward.approve` — default to **adult = all on (parent), teen/kid = off** and are owner-editable per household (`households.settings.permissions`, deep-merged over defaults, **no migration**); `is_admin` stays the always-everything superuser. Backend: `platform/permissions.ts` (`can`/`resolveCapabilities`/`requireCapability`); `Tenant` gains `memberType`; `requireAdmin` → `requireCapability` on chore approve/reject, chore edit/delete, reward CRUD, and redemption approve/deny. Chore creation keeps a **carve-out**: anyone may create a chore for *themselves* or *up-for-grabs* (`person_id` null/self); assigning to someone else needs `chore.manage`. New `GET/PUT /api/permissions` (admin); `/api/household` returns the caller's resolved `capabilities`. Web: a `can(person, cap)` helper drives **render-if-capable** gating (no more show-then-403) across Tasks/Approvals/RewardsPanel, the chore assignee picker limits to self/up-for-grabs without `chore.manage`, and **Settings → Family & People** gets a 3×4 permissions grid. Photo-wall/media uploads stay open to any member (no admin gate). Unit + integration tests; verified live with Playwright (adult non-admin approves/manages; kid blocked 403 + controls hidden; carve-out + grid persistence). iOS unaffected.
- [x] 3.4a **Permission ideology + goals integrity + list attribution** (DONE 2026-06-24). Codified the rule we apply to every write — **touches currency or someone else's record → gate; collaborative but you'd want to know who → attribute; neither → leave open** — in [`website/docs/src/content/docs/concepts/permissions.md`](website/docs/src/content/docs/concepts/permissions.md). Applied it to two more domains (still **no migration** — caps in `settings.permissions`, attribution columns already existed): **(1) Goals integrity** — fifth capability `goal.manage` (adult on, teen/kid off; admin superuser). Closed the hole where any member could attribute progress to anyone: logging a target other than self, creating a goal that assigns *other* participants, editing/deleting a shared/others' goal, and goal-list edit/delete now need `goal.manage`. Carve-outs stay open: logging **your own** progress (or a family/shared log), creating a **personal** goal (sole/no participants), editing/deleting **your own sole-participant** goal, ticking a checklist step (self-attributed), and creating a goal list. `goalParticipantIds()` helper; route-layer gating only (the calendar→goal bridge logs via the service, unaffected). Web mirrors the chores idiom: the log-progress "who took part" picker and create-goal participant picker limit to self without the cap, and edit/delete affordances render only when capable (or it's your own solo goal); the Settings grid is now 3×5. **(2) List/meal attribution** — list items already stored `created_by`/`source`/`source_recipe_ids`; surfaced them through `presentListItem` as `addedBy`/`source`/`sourceRecipeIds`, and the grocery/Lists UI shows **"added by {name}"** on manual items and **"🍽 from meal plan"** on auto-built ones (no gate — collaboration stays open). Recipes/meal-planning stay fully open by design. Tests: goals gating integration (9) + permissions unit + lists attribution + web gating/render suites. **Not committed (recorded, not built):** a single soft "kids' list additions need an OK" household toggle — attribution covers the real need; a per-role lists matrix is more machinery than a family hub wants.
...
- **Local-only, buildable now:** **6.7 kiosk reminders** (local "due soon" banner +
  `reminders` table — table not built yet),
  ...
  *(DONE: **recurring events** end-to-end — RRULE picker + per-occurrence/this-and-following/all
  edits, expansion read-model `event_occurrences` (migrations 0048–0050), Google recurrences
...
- [x] **List templates** *(web SHIPPED · iOS parity SHIPPED)* — save a list (e.g. "Beach Day") as a
  ...
  provenance) rather than new tables. Service: mirror the item-copy in `addRecipeToGrocery` +
  cascade pattern in `softDeleteList` (`lists.service.ts:264,92`), inserting items with
...
- [x] **Markdown timer parsing** *(web/backend SHIPPED · iOS needs no client work)* — the deferred
  ...
  inline token (mirror the `**Ingredients:**` split at `:153`), wire it into the importer insert
  (`import-recipes.ts:77-80` currently omits it), and document the syntax in `docs/RECIPE_FORMAT.md`
...
- [ ] **Countdown tweaks** *(web SHIPPED · iOS parity STILL PENDING)* — polish on the shipped countdowns
  ...
  instead of a raw checkbox). (b) **Birthday horizon:** `listCountdowns` pushes *every* birthday with
  no filter (`countdowns.ts:84-93`), and `nextBirthday` rolls an already-passed birthday to next year
  — gate the push on `daysBetween(today,date) <= ~183` (≈6 months) / same calendar year, so a
  year-away birthday hides until it's close. (c) Optional `birthdayHorizonDays` setting in
...
- [x] **Waffles default recipe (Easter egg)** *(backend seed SHIPPED; surfaces on web + iOS)* — a
  ...
  reusing `parseRecipe` + the meals insert helpers — doubles as a canonical example recipe and a
...
- [x] **Spot-award stars** *(web SHIPPED · iOS parity SHIPPED)* — a parent hands out ad-hoc stars
  ...
  `reward.grant` capability (`permissions.ts:10-21`, adult on / teen+kid off / admin superuser), a
...
Design rationale + the A/B/C pattern model: [`website/docs/src/content/docs/concepts/extensibility.md`](website/docs/src/content/docs/concepts/extensibility.md).
...
Design rationale + web/iOS model: [`website/docs/src/content/docs/concepts/extensibility.md`](website/docs/src/content/docs/concepts/extensibility.md).
...
- **Food / pantry inventory** — SHIPPED (web) 2026-06-30 → 2026-07-01 (build #2). Track what's on
  ...
  (barcode/scanner scripts). See [`website/docs/src/content/docs/reference/features.md`](website/docs/src/content/docs/reference/features.md) (Pantry rows).
...
- **list-members** — people + login email, admin/owner, password/SSO presence.
- **make-admin** / **revoke-admin** `(--email | --person)` — grant/revoke admin (owner protected).
- **password-login `<on|off>`** — flips `auth_config.password_login_enabled` (the Settings toggle).
- **clear-calendar-error** `(--email | --all)` — clears a stuck Google account's `last_sync_error`
...
- [x] Recurring events — per-occurrence tag + confirmation. The recap queue UNIONs
      `event_occurrences` for masters with a `goal_id` (no longer gated to `rrule is null`);
      `confirmRecap` keys idempotency on `(event_id, occurrence_date, goal_id)`. *(was Phase 2)*
...
- [x] **Docs site (Astro Starlight)** — SHIPPED 2026-07-01. `website/` holds the user/operator
  docs (Getting started · Features · Concepts · Operations) with built-in offline search; the
  markdown moved out of `docs/` (site is canonical; engineering docs stay in `docs/`). Deploys to
  GitHub Pages via `.github/workflows/docs.yml`. *(Caveat: root hosting — a custom domain or an
  `<owner>.github.io` page — works fully; a project subpath would need base-aware in-body links.)*
