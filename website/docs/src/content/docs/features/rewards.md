...
- 🔒 **Capability-gated where it has stakes** — `reward.manage` (catalog, currencies, conversions), `reward.approve` (decisions and redeeming/converting for another person), `reward.grant` (spot-awards). Anyone may **redeem for themselves** and convert their own balance. Pending rewards require a different person to approve them, including requests made by an admin or on a child’s behalf. See [Permissions](/concepts/permissions/).
