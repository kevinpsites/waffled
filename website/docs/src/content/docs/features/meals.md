...
- ✏️ **In-app recipe editor** — metadata, ingredients, and steps, with per-step ingredient amounts and ingredient **sections** you can drag items between. Set a photo by URL or upload, and **remove it** with the trash button next to the photo (delete-recipe is web-only). Quantities take fractions the way recipes are written — `1/2`, `1 1/2`, `⅛` — and come back out as the fraction you typed rather than a decimal.
- 🛒 **Add to grocery, your way** — "Add to grocery" opens a picker so you can add all of a recipe's ingredients or just the ones you're missing. With [Pantry](/features/pantry/) on, anything it finds on your shelves starts **unchecked** (and the picker says how many); staples stay checked but are marked "likely on hand" so you can drop what's already in the cupboard. See [Pantry-aware shopping](/features/lists/#pantry-aware-shopping).
