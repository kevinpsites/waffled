...
- ✏️ **Fix a mistaken entry** — every line in a goal's Recent activity can be **edited** (amount, **who took part**, note, date) or **deleted**; a shared/split entry is removed as a whole and re-splits correctly when you change who was there. An entry that counted *itself* — a checklist tick, a confirmed calendar event, an Apple Health sync — keeps its amount and date in step with whatever logged it, so the sheet offers just a **note**; change the tick, the event, or Health to change the rest.
...
- 🗓️ **A target for this week** — set one in [Weekly Planning](/features/weekly-planning/)'s Goals step ("10 hours this week toward 750") and the goal's card on the Goals page and the Today goal card show it: **This week: 3 of 10 hours**, or **Week of Sep 21: 10 hours** until that week starts. On the goal's own page, the **This week** corner reads **3 of 10 hours** and each week planned after it is listed beneath. Running counts and totals take one; habits already have a per-period target.
...
- 📅 **Calendar auto-count** — a single-event recap, smart **"might count toward a goal"** suggestions that learn over time (and that you can tell to **ignore events with a word** for a goal), and recurring-event counting (via Calendar's **"counts toward a goal"** tag). Confirming a recap counts it on **the event's day**, so answering a few days late doesn't move the entry to today.
...
- 🔁 **A habit counts the period it's in, and resets with it.** "5× a week" shows the days you've logged *this* week against the five — not a lifetime tally — and a weekly habit starts over on your household's own **Week starts on** day (**Settings → Family & People**), the same Sunday or Monday the meal planner and calendar are cut on. Daily habits reset at midnight, monthly ones on the 1st, both in your household's timezone. Logging twice in one day counts once, by design — the Log sheet says **Already submitted today** once it's done (per person, or for the family on a habit nobody is picked for), and you can still pick an earlier day to catch one up.
- 🙈 **Stop a suggestion that keeps coming back.** Dismiss hides one event. When the same kind of event is suggested every week, choose **Ignore…** (web) or **Dismiss → Ignore events like this…** (iPhone/iPad) and pick a word from its title — say *thaw* — and events containing that word are never suggested for that goal again. Other goals still see them. Remove a word in **Settings → AI & Capture → Ignored for suggestions** (web, iPhone and iPad). Waffled's own planned-meal and thaw reminders are never suggested at all.
