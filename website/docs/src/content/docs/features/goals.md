...
- 🙈 **Stop a suggestion that keeps coming back.** Dismiss hides one event. When the same kind of event is suggested every week, choose **Ignore…** (web) or **Dismiss → Ignore events like this…** (iPhone/iPad) and pick a word from its title — say *thaw* — and events containing that word are never suggested for that goal again. Other goals still see them. Remove a word in **Settings → AI & Capture → Ignored for suggestions**. Waffled's own planned-meal and thaw reminders are never suggested at all.
