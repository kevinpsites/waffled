...
| Variable | Purpose | Default |
|---|---|---|
| `POWERSYNC_PUBLIC_URL` | The sync URL clients connect to. **Leave empty on a LAN** — Waffled derives it from the address each device used. Set it only when PowerSync has its own public hostname/TLS | empty (derived) |
