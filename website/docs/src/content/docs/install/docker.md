...
| **caddy** | `waffled-caddy` | Reverse proxy **and** web server — the React SPA is baked into this image. Proxies `/api/*` → api, validates signed `/media/*` bearer URLs, SPA fallback. |
