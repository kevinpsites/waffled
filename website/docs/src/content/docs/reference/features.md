...
| **Completed section** on a custom list — checked items tuck into a collapsible "Completed" group (with an undo grace window) instead of lingering in place. Checked items **auto-clear ~24h** after check-off (per-list `auto_clear_checked`, default 24h; grocery is exempt — its checked = in-cart). A list's item count reflects only **unchecked** items | ✅ | ✅ | ✅ | ✅ Done — web + iOS; auto-clear runs lazily on load (no cron), scoped to `list_type='custom'` |
| **Bulk-edit items** — enter Select mode, pick multiple items, and set their **section / assignee / priority** in one action (`PATCH /api/list-items/bulk`, household-scoped) | ✅ | ✅ | ✅ | ✅ Done — web + iOS |
| **Collapsible sections** on a custom list — collapse/expand each section from its header | ✅ | ✅ | ✅ | ✅ Done — web now matches iOS |
| **Sticky add section** — the add bar's section picker keeps its choice across quick adds, so a run of items lands in the same section | ✅ | ✅ | ✅ | ✅ Done — web now matches iOS |
