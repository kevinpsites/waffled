...
| **Register screen** — grouped by urgency, with current-period state per rhythm | ✅ | ✅ | ✅ | ✅ Done on web and iOS — **Needs you now / Coming up / Steady**, paused named in a line at the bottom, each row anchored by a countdown (*6 days late*, *3 months*, **Booked**) over a hairline for how much of the cycle is spent, soonest-first inside each group. "Needs you now" is the same `/attention` list the Today card reads, so the two can never disagree about one rhythm. The shapes are no longer named as headings — they show up in the row's own words (*last done Aug 19* vs *not on the calendar yet*) and its verb (**I did it** vs **Book a time**). iOS matches, from the same helpers, and adds booking a period whose runway hasn't opened to the row menu — the case `/attention` structurally cannot report. The per-row nudge setting is gone from both: repeated on every row it was noise, and it lives in the editor where the number is chosen |
