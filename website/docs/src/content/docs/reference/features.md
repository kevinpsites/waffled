...
| **Rhythms** module — the standing intentions that should keep happening (air filter every 3 months, trash weekly, a quarterly temple visit), with a Today card, a register, period booking into real calendar events and a 🔁 marker on the events that keep one. See [Rhythms](#rhythms) above | ✅ | ✅ | ✅ | ✅ Done — opt-in in Settings → Modules. Everything ships on all three surfaces except the 🔁 event marker, which is web/kiosk only so far |
