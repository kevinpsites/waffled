...
| **Search a list** — filter a long list down to what you're looking for by item name, section, or quantity | ✅ | ✅ | ✅ | ✅ Done — web now matches iOS. Local filtering (no reload), matching the same three fields on every surface. A search **overrides collapsed sections and the Completed group** so a match can't hide behind a header the user closed earlier; the header's item count and **Share list** still describe the whole list, not the filtered view. Web: a search field in the list header (clear with **×** or Esc); iOS: a search field above the list once it's longer than six items |
