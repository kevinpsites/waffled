...
| **Edit / pause / retire a rhythm** | 🚧 | 🚧 | 🚧 | 🟡 Server-side only — `PATCH /api/rhythms/:id` (title · emoji · notes · assignee · cadence · lead time · `isActive`) and a soft `DELETE` exist and are tested, but **no client exposes a control yet**, so the register's `Paused` badge is currently unreachable. `satisfiedBy`, `startsOn`, `autoSchedule` and `rrule` are refused even there: re-anchoring a live rhythm would re-interpret existing skips (keyed on `period_start`) and point bookings at periods that no longer exist |
