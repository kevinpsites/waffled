...
| **Capability gating** — `reward.manage` / `reward.approve` / `reward.grant`; anyone may redeem for **self**, but redeeming (or converting) on behalf of **another member** needs `reward.manage` — enforced on the REST route and the capture bar alike | ✅ | ✅ | ✅ | ✅ Done |
...
| **Nudge runway** (`leadTime`) — "warn me before it's due" / "how many days' warning before the booking window closes" | ✅ | ✅ | ✅ | ✅ Done — clamped server-side, with a ceiling that differs by shape because only one shape has a floor: a **scheduling** rhythm gets the **whole cadence** (its feed stops asking when the window closes, so a runway equal to the cycle opens on the period's first day and shuts on its last — which is what makes *"remind me on the 1st to plan the outing, I'll book it for whenever suits"* sayable at all), a **completion** rhythm keeps **half** (its feed has no upper bound on purpose, since an overdue thing can still be done, so a full-cycle runway would surface it the instant it was completed and never let it go quiet), and a **booking window** is the ceiling wherever there is one. Longer than the ceiling is refused either way. The clients send the **cadence interval** rather than a day count when the runway covers the whole cycle — `30 days` is a month only in a 30-day month, so a monthly rhythm asked to open on the 1st opened on the 2nd in a 31-day one. The editor states the runway you'll actually get in days. It **follows the cadence** — half the cycle, capped at a fortnight — until a number is typed, rather than defaulting to a flat 14 that the server then trimmed without saying so. A typed number is still sent as typed, with the clamp admitted next to it in both numbers |
...
| **Mark a period handled** (`POST /:id/skip`) | ✅ | ✅ | ✅ | ✅ Done — sends one period quiet without inventing a calendar entry, whether it was sorted another way or simply isn't happening. It settles the period the same way a booking does, so the list reports it as satisfied with **no booked time** — which is how a row tells "on the calendar at 2pm" apart from "handled without a booking". Records **no completion** and enters no history: a scheduling rhythm never asks whether the thing happened, so a word like "Done" would claim an answer the server doesn't have, and "Skip" (what this was called) named only the not-happening case and hid the commoner one. The route and table keep the `skip` name — the wording changed, the semantics did not |
...
| **Search a list** — filter a long list (or the grocery board) down to what you're looking for by item name, section/aisle, quantity, or store | ✅ | ✅ | ✅ | ✅ Done — web now matches iOS, on **both** the custom-list view and the grocery board (where the search works in By aisle / By store / By meal alike). Local filtering, no reload; the fields matched are the ones the row shows. A search **overrides collapsed sections and the Completed group** so a match can't hide behind a header closed earlier — and those headers go inert while searching, so clearing the box restores exactly the collapse state the user left. The header count, **Share list** and the grocery board's **Clear** still act on the whole list, never the filtered slice. Web: a search field in the header (clear with **×** or Esc); iOS: a search field above the list once it's longer than six items |
...
| **Meal Builder** — build one meal out of several recipes (a "plate") | ✅ | ✅ | ✅ | ✅ Done — name it, add recipes under **Main / Sides / Dessert**, set meal-level servings, assign **a cook per dish**. Saved meals are first-class in the recipe library (a `Meal · N` badge, a 🍽️ **Meals** filter, search spans the meal's name *and* its dish titles); adding a saved meal to a plate flattens it into its dishes. Adding a dish is a tap through the recipe/meal picker on every platform; a dish already on the plate can be **dragged between roles** (on iPhone/iPad via a press-and-hold over one flat run of rows — a `List` reorders only within a section and refuses `.dropDestination`). **A plate can also be built from inside the recipe picker** — wherever that picker can take a whole meal back, ＋ opens a menu with **New meal** beside **New recipe**, which raises this same builder over the picker; **Use this meal** drops the finished plate straight on the night you were filling and saves it to your library on the way past. On the web that is every dish picker: the meal planner's night picker and Weekly Planning's Meals step. On iPhone/iPad it is Weekly Planning's Meals step and the builder's own "add a side" — the Meals tab's own night picker still takes a single recipe only (it shows no saved plates either), so it keeps the plain ＋ New recipe rather than offering to build something the slot can't accept |
...
| **Weekly Planning** module — a guided session that decides the week ahead in ten steps (loose ends · calendar · horizon scan · family night · connection · goals · meals · tasks · kids · recap). Settings → Modules sets the session's day/time and turns individual steps off; the **Planning** page holds the lobby, the step chrome (counter → agenda sheet, the one question each step asks, skip/affirm footer) and the saved record of what the week decided — which opens with the week itself (the seven days with their dinners and events, what the session changed grouped by the module it lives in, the notes nobody tagged, and what was left alone on purpose), with the per-step tick-list underneath as the record of what was skipped on purpose. Defaults to the week ahead, with a stepper for planning any later week (each week gets its own session and record); every step has its own URL, so leaving and returning — or a refresh, or the back button — resumes exactly where you were, with **Leave for now** in the session header (and in the agenda sheet), which returns you to the lobby holding the part-planned session — resume it, or step to another week — rather than resuming it automatically on the next visit, and **Start this week over** in the sheet (starting over discards the session record only — what it already decided stays in the calendar/chores/goals) | ✅ | ✅ | ✅ | ✅ Web and iOS both complete — the **shell** (module toggle + settings panel, the `/planning` lobby, the session chrome, the saved record, and the parked-note handoff that opens each step with whatever was tagged for it — the step's own verb where it has one (**Make a task**, **Make an event**, opening that step's existing composer with the note's words in it), plus **Edit**, which fixes the note's wording or re-addresses it to a different step — a re-tagged note moves its routing with it, so the note and step 1's receipt never disagree — and Handled / Drop it / leave it parked) plus **all 10 steps**: **Loose ends** (what's still open + parked notes, each row naming the source it came from — chore · list · rhythm · goal — and the person who already has it where anything does, so an unowned row reads as up for grabs; each routed to the step that will handle it — routing writes nothing to your modules; overdue chores, late rhythms and short habits always count, while **which of your lists it asks about is yours to choose** — **Which lists?** in the step itself opens a switch per list, so a list meant to stay open (a someday list, a wishlist) stops coming up every session and the deck re-reads on the spot. Gated by `planning.manage`, which **every adult holds by default** — running the session is not an admin act — and which an admin can grant to a teen in the permissions grid; the same switches are in Settings → Modules → Weekly Planning for setting it up outside a session. A list you say nothing about keeps counting, and your grocery list is never asked about: it rebuilds itself from the meal plan), **Calendar** (the real week, add-in-place on a tapped day), **Goals** (a tab per goal list, focus via the existing featured flag, pins you made by hand are never cleared, and **＋ New goal for this week** opening the app's own goal editor over the session — fixed to the group whose tab you're on and pinned on the way in, so it comes back as that group's suggested focus rather than needing a second trip; adding to a group that isn't just you needs permission to manage goals), **Meals** (dinners in the same seven columns; **✨ Plan the rest** opens the app's own "Plan my week" planner — its guardrails, the preferences box, reshuffle, swap, lock and a manual pick per night — narrowed to the nights with nothing on them, so approving a week fills only those and never overwrites a night somebody already decided, and **Undo the three** takes back exactly what it wrote and reports any night decided since instead of clearing it; a night is also fillable by hand with a recipe **or a saved plate** — built in place with the real Meal Builder if it doesn't exist yet — and the shopping trip is assignable as a real chore), **Tasks** (per-person columns, an up-for-grabs strip, drag or tap to hand a chore over, and the existing chore modal for both adding — Who prefilled — and editing, reached from a task's name or the day beside it). **Horizon scan** (the shipped month view plus one bar that parks a note — never a calendar entry — tagged for one of the steps still *ahead* of you, which then opens with it; an untagged note stays on the board for the recap and next week's loose ends, and the bar says so; anything already on the **Parked in this session** board is editable in place — **Edit** beside it changes the wording, the tag, or both), **Family night** (the rotation's suggestion per part, pinnable for this week only, a line per part for what it actually is, an optional theme, getting *this* week onto the calendar as a new event or by pointing at one already on the week, and calling the week off without touching the standing recurring event), **Connection** (pairings read out of `event_participants`, time you are *already* spending together counted rather than replaced — including **Link a time**, which lists every event on the week with both of you on it, whoever else is there too, and lets you say that one *is* your time without editing anybody's event — and any pairing makeable from scratch, every one landing as an ordinary calendar event), **Kids** (a card per child with their week, stars, a focus drawn from their own goals and overdue chores, and something to look forward to picked off their own calendar, then read back to them — the focus also surfaces on that child's Family profile as "this week's one thing", read from the session rather than copied). **Recap** (the week read back one last time, with what the session changed grouped by the module each decision lives in — every line a pointer to live data rather than a copy — plus a last call on notes nobody tagged and a column for what was left alone on purpose). On iOS the session is reached from the **Weekly Planning** tile in the Family tab and from the iPad display's own **Planning** rail page — plus a Today card **on the display** (the iPhone's Today has no planning card; `Settings → Weekly Planning → Show on the family display's Today` governs the display's) — Settings → Weekly Planning *configures* the session (day, time, which steps run, and which lists step 1 asks about) rather than starting one; the phone's bottom bar is full, so there is no planning tab. A step whose module is off is skipped automatically and never counted, so the counter reads "3 of 7" for a household that doesn't use meals |
...
| **"Can't reach the server" banner** — the device is online but Waffled isn't answering | ✅ | ⬜ | ⬜ | ✅ Done (web/kiosk) — a stopped server (Mac app quit, Docker stack down) used to leave every screen blank with nothing to explain it, since `navigator.onLine` was still true. A strip on **every** screen — including sign-in and the kiosk profile picker — names the outage, says how to start the server again (menu-bar waffle on a Mac, `./waffled status` on Docker) and offers **Retry**; the sign-in screen says the server isn't answering instead of offering a login that can't succeed, and keeps checking on its own. Failed requests count, and so do gateway answers (502/503/504) that aren't Waffled's own — an error Waffled itself chose to send (a recipe import that failed, a barcode lookup that timed out) is proof it's alive and never raises the banner. A short grace stops blips flashing it, and a background probe every 5s (15s after a minute) refreshes every open screen the moment the server is back. A device that is itself offline wins immediately: the strip switches to saying you're offline — no Retry, no advice about starting a server that's fine — and hands the story to the Offline banner once the disconnection has lasted long enough to be real; the sign-in screen says the device is offline too |
...
| **OpenTelemetry** traces+metrics (OTLP, **off by default**) | ✅ | — | — | ✅ Done — Docker installs only; Waffled for Mac doesn't include it yet |
| All-local **Grafana/OTEL stack** (`./waffled observability up`, profile) | ✅ | — | — | ✅ Done |
| **Waffled for Mac** — a downloadable, notarized menu-bar app that runs the whole server natively (no Docker): a guided first run that settles the data folder, nightly backup, network address and port — and anything else Settings has — before anything is created, a `Settings…` that reopens the same screen afterwards and applies only what changed — both in Basic / Advanced / Diagnostics tabs covering backup time and retention, the AI provider (Claude, OpenAI-compatible, or Ollama detected on the Mac), AI limits, calendar-sync OAuth, sessions, rate limits and logging — moving the whole household to another folder, plus start-at-login, backups, and self-updating | ✅ | — | — | ✅ Done — Apple silicon, macOS 14+. The app is a server manager, not a second UI: the interface stays the web app it serves. Same api, migrations, PowerSync and web build as Compose, which remains the way to run Waffled on Linux/NAS/Pi/VPS. See [Mac install](/install/mac/) |
