...
| **"Can't reach the server" banner** — the device is online but Waffled isn't answering | ✅ | ⬜ | ⬜ | ✅ Done (web/kiosk) — a stopped server (Mac app quit, Docker stack down) used to leave every screen blank with nothing to explain it, since `navigator.onLine` was still true. A strip on **every** screen — including sign-in and the kiosk profile picker — names the outage, says how to start the server again (menu-bar waffle on a Mac, `./waffled status` on Docker) and offers **Retry**; the sign-in screen shows the same thing instead of a login that can't succeed. Failed requests and gateway answers (502/503/504) both count, a short grace stops blips flashing it, and a background probe every 5s (15s after a minute) refreshes every open screen the moment the server is back. Device-offline still wins and shows the Offline banner |
