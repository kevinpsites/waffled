...
| **Public API keys + scopes** — `waffled_…` key, `x-api-key`, `<resource>:read\|write` scopes | ✅ | ❌ N/A | ❌ N/A | ✅ Done (web; build #3) — external-integration surface (pattern B), admin-issued; `lists:write` keys can add, update, and remove list items |
