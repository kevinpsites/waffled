...
| **Waffled-Bites** module — pair a kid's companion touchscreen device (Family → kid → Waffled-Bite) and control it live: quiet-time countdown (start/pause/+5/end), night light (color + brightness), per-day wake-up light schedule (yellow "almost time" warning + green go-time), morning alarm, sound machine, screen brightness, and quick nudges | ✅ | ❌ N/A | ❌ N/A | 🟡 Partial — pairing + parent control panel done (web); the physical device (ESP32 touchscreen) and its on-screen app are a separate, still-in-development embedded project, so there's nothing to pair with yet; iOS control panel not yet built |
