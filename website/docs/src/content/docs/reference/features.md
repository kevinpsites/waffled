...
| **Do things**, not just add ("mark the trash chore done", "give the dishes to Wally", "log 20 min on my reading goal") | ✅ | 🚧 | 🚧 | 🟡 In progress — Tier 2 mutate verbs. Resolves the thing by description (pick-one when ambiguous), server-only, destructive actions confirm. Shipped: **complete / reassign a chore** + **log goal progress** on web. Rescheduling events, list check-off, reward redeem, and iPhone/iPad are fast-follows |
