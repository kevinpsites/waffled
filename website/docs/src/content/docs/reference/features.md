...
| **Re-anchoring is refused** — `satisfiedBy`, `startsOn`, `autoSchedule` and `rrule` can't change on a live rhythm | ✅ | ✅ | ✅ | ✅ Done (server) — it would re-interpret existing skips (keyed on `period_start`) and point bookings at periods that no longer exist. Retire it and make a new one |
