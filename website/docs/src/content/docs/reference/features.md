...
| **Rhythms card** on Today — what needs attention today, overdue first; **Mark done** on a completion rhythm, **Book a time** / **Skip** on a scheduling one | ✅ | ✅ | ✅ | ✅ Done — renders **nothing** on a quiet day rather than an empty card (same rule as Tonight with no dinner planned), since a quarterly register is silent most mornings. Module-gated and default-off, so it is injected into the board rather than living in saved layouts; the label still appears in Customize so it can be hidden and brought back |
