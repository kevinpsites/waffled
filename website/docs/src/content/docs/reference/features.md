...
| **Rhythms card** on Today — a countdown block: *"3 want attention"* + *"All 10 →"*, each row led by its countdown (*7 days late · every 3 months*), the filled button kept for what is late or out of time; **I did it** on a completion rhythm, **Book** / **Skip** on a scheduling one | ✅ | 🟡 | 🟡 | ✅ Done on web — renders **nothing** on a quiet day rather than an empty card (same rule as Tonight with no dinner planned), since a quarterly register is silent most mornings. Module-gated and default-off, so it is injected into the board rather than living in saved layouts; the label still appears in Customize so it can be hidden and brought back. The register total in the header costs a second request, so it is only asked for on days the card actually renders. iPhone/iPad still show the older card |
...
| Create a rhythm — title · emoji · cadence (every N days/weeks/months/years) · assignee or whole household · anchor · notes | ✅ | ✅ | ✅ | ✅ Done |
...
| **Auto-schedule** a scheduling rhythm (the cadence fully determines when → one recurring event, created once) | ✅ | ✅ | ✅ | ✅ Done — the series is booked **when the rhythm is created** (6pm in the household's timezone, on the first day the rule allows at or after the start date — so an anchor and a chosen weekday that disagree don't produce a master on a day its own rule excludes), so it is on the calendar from the outset rather than waiting to be put there. Weekly rhythms pick the day with the calendar's own weekday chips (single-select: a rule firing twice a period would assert something the cadence never said); monthly rhythms pick same-date / same-weekday / last-weekday. **Advanced (raw RRULE)** is the escape hatch, behind a disclosure |
| **Nudge runway** (`leadTime`, default 14 days) — "warn me before it's due" / "how many days' warning before the booking window closes" | ✅ | ✅ | ✅ | ✅ Done — clamped server-side to at most **half the cadence** (a runway longer than the cycle never closes, so the item would never go quiet); the editor states the runway you'll actually get in days |
| **Book a period** → a real calendar event carrying `rhythm_id` | ✅ | ✅ | ✅ | ✅ Done — date + time + all-day only; title and assignee come from the rhythm. The event is ordinary, so it inherits recurrence, colors, participants, visibility, reminders and Google/Outlook sync for free. The picker is clamped to the period (a booking outside it satisfies the wrong one) |
| **Skip a period** | ✅ | ✅ | ✅ | ✅ Done — sends one period quiet without inventing a calendar entry for something that isn't happening |
...
| **Register screen** — grouped by urgency, with current-period state per rhythm | ✅ | 🟡 | 🟡 | ✅ Done on web — **Needs you now / Coming up / Steady**, paused named in a line at the bottom, each row anchored by a countdown (*6 days late*, *3 months*, **Booked**) over a hairline for how much of the cycle is spent, soonest-first inside each group. "Needs you now" is the same `/attention` list the Today card reads, so the two can never disagree about one rhythm. The shapes are no longer named as headings — they show up in the row's own words (*last done Aug 19* vs *not on the calendar yet*) and its verb (**I did it** vs **Book a time**). iPhone/iPad still group by shape |
| **Edit / pause / retire a rhythm** | ✅ | ✅ | ✅ | ✅ Done — edit covers title · emoji · notes · assignee · cadence · nudge runway; **Pause** stops a rhythm asking without losing anything, and **Retire** removes it while keeping its completion history. `satisfiedBy`, `startsOn`, `autoSchedule` and `rrule` are deliberately not editable: re-anchoring a live rhythm would re-interpret existing skips (keyed on `period_start`) and point bookings at periods that no longer exist, so the editor explains the anchor instead of offering to move it |
...
| **Choose which ingredients to add** — "Add to grocery" opens a picker to add all or just the ones you need, instead of always adding everything. Every ingredient starts checked; pantry staples are flagged "likely on hand" to steer what you uncheck | ✅ | ✅ | ✅ | ✅ Done — web + iOS; `POST /api/lists/grocery/from-recipe/:id` takes an optional `ingredientIds` subset (both clients always send one; omitting it keeps the older add-all-non-staples behavior) |
