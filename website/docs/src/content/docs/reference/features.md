...
| **Auto-schedule** a scheduling rhythm (the cadence fully determines when → one recurring event, created once) | ✅ | 🟡 | 🟡 | 🟡 The behaviour is done everywhere; the **editor's controls for it are web-only**. On every surface the series is booked **when the rhythm is created** (6pm in the household's timezone, on the first day the rule allows at or after the start date — so an anchor and a chosen weekday that disagree don't produce a master on a day its own rule excludes), so it is on the calendar from the outset rather than waiting to be put there. On web, weekly rhythms pick the day with the calendar's own weekday chips (single-select: a rule firing twice a period would assert something the cadence never said), monthly rhythms pick same-date / same-weekday / last-weekday, and **Advanced (raw RRULE)** is the escape hatch behind a disclosure. On iPhone/iPad the weekday is taken from the start date instead, monthly offers same-date / same-weekday only, and there is no raw-rule escape hatch — so a rule the pickers can't express has to be authored on the web |
