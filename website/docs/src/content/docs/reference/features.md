...
| **Remove an off-plan recipe from the grocery list** — undo the above; the recipe drops out of the by-meal "Unscheduled" shelf, keeping any items it shares with another recipe | ✅ | ✅ | ✅ | ✅ Done — web: **Remove** on the Unscheduled section; mobile: long-press the section → **Remove from list** (`DELETE /api/lists/grocery/from-recipe/:id`) |
