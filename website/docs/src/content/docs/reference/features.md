...
| **Push a completion rhythm out** ("push it out a week") | 🟡 | 🟡 | 🟡 | 🟡 Server done — `PATCH /api/rhythms/:id` takes `nextDueAt` for the completion shape only, and refuses it on a scheduling rhythm, whose periods *are* its anchor and whose skips are keyed on them. It is one period's reprieve rather than a permanent shift: the next completion re-anchors from when you actually did it. No control on any surface yet |
| **Completion history + the real average** | 🟡 | 🟡 | 🟡 | 🟡 Server done — `GET /api/rhythms/:id/completions` returns a page (50 by default, 200 ceiling) newest-first, plus `total` and `averageIntervalDays`. The average is taken over **every** completion rather than over the page: derived from the most recent 50 it would be a *recent* average wearing the wrong label, and that only becomes visible to someone with years of history. Null from fewer than two — one date is not an interval. The web client has the call and no UI using it; iOS has neither |
