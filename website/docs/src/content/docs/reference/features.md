...
| **Auto-schedule** a scheduling rhythm (the cadence fully determines when → one recurring event, created once) | ✅ | ✅ | ✅ | ✅ Done — the series is booked **when the rhythm is created** (6pm on the start date, in the household's timezone), so it is on the calendar from the outset rather than waiting to be put there. Weekly rhythms pick the day with the calendar's own weekday chips (single-select: a rule firing twice a period would assert something the cadence never said); monthly rhythms pick same-date / same-weekday / last-weekday. **Advanced (raw RRULE)** is the escape hatch, behind a disclosure |
