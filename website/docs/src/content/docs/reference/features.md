...
| **Waffled for Mac** — a downloadable, notarized menu-bar app that runs the whole server natively (no Docker): a guided first run that settles the data folder, nightly backup, network address and port before anything is created, a `Settings…` that reopens the same screen afterwards and applies only what changed — in Basic / Advanced / Diagnostics tabs covering backup time and retention, the smart-suggestions provider (Claude, OpenAI-compatible, or Ollama detected on the Mac), AI limits, calendar-sync OAuth, sessions, rate limits and logging — moving the whole household to another folder, plus start-at-login, backups, and self-updating | ✅ | — | — | ✅ Done — Apple silicon, macOS 14+. The app is a server manager, not a second UI: the interface stays the web app it serves. Same api, migrations, PowerSync and web build as Compose, which remains the way to run Waffled on Linux/NAS/Pi/VPS. See [Mac install](/install/mac/) |
