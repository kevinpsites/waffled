...
| **Auto-schedule** a scheduling rhythm (the cadence fully determines when → one recurring event, created once) | ✅ | ✅ | ✅ | ✅ Done on every surface, editor included. The series is booked **when the rhythm is created** (6pm in the household's timezone, on the first day the rule allows at or after the start date — so an anchor and a chosen weekday that disagree don't produce a master on a day its own rule excludes), so it is on the calendar from the outset rather than waiting to be put there. Weekly rhythms pick the day with the calendar's own weekday chips (single-select everywhere: a rule firing twice a period would assert something the cadence never said, and one booking settles it either way); monthly rhythms pick same-date / same-weekday / last-weekday; **Advanced** is the raw-RRULE escape hatch behind a disclosure. Leaving the chips alone follows the start date, which is the default rather than the only option |
