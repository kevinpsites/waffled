...
| **Share list** — hand any list to a phone (text / share sheet / QR) | ✅ | — | — | ✅ Done (web/kiosk; grocery board + custom lists. The QR encodes the list text itself, so no app or account is needed on the phone; a list too long to encode legibly falls back to Copy / Share) |
...
| **Meal Builder** — build one meal out of several recipes (a "plate") | ✅ | 🚧 | 🚧 | ✅ Done (web) — name it, drag recipes in under **Main / Sides / Dessert** (drag between roles, or ＋ files by the segment you're browsing), set meal-level servings, assign **a cook per dish**. Saved meals are first-class in the recipe library (a `Meal · N` badge, a 🍽️ **Meals** filter, search spans the meal's name *and* its dish titles); dropping a saved meal on a plate flattens it into its dishes. Mobile parity is next |
| Meal Builder — **schedule a meal** to a night, or **add it to the list** without scheduling | ✅ | 🚧 | 🚧 | ✅ Done (web) — scheduling fills that day+slot (`meal_plan_entries.meal_id`), goes on the calendar as one event and feeds the weekly grocery rebuild; "Add plate to list" only shops for it (`source='recipe'` rows the rebuild never wipes) and can be **taken back off** again — rows the week's own plan still needs survive |
| Meal Builder — **the meal's shopping, grouped as one meal** | ✅ | 🚧 | 🚧 | ✅ Done (web) — the grocery board's by-meal view groups a plate's items under the plate (badged **Unscheduled** when it isn't on a night), one dot per row in the plate's colour, one row per item however many dishes want it. A plate whose items are all claimed by an earlier meal keeps its heading and says where they went |
| **Cook a whole meal** in cook mode (tabbed across its dishes) | ✅ | 🚧 | 🚧 | ✅ Done (web) — `/meals/meal/:id/cook` tabs across every dish with one shared timer dock, so timers started on one dish keep running while you read another. Reachable from the meal itself and from the Tonight card |
| **Which ingredients are still to buy** (not just how many) | ✅ | 🚧 | 🚧 | ✅ Done (web) — the recipe detail's on-hand banner and every dish on a plate expand "N to buy" into the ingredient names (`toBuyNames`). With the Pantry module on these are the *unmatched* ones specifically, which no client could derive from the ingredient list alone |
