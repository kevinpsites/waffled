...
| **Mark a completion rhythm done** (any time, not just when due), and **backdate one** | ✅ | ✅ | ✅ | ✅ Done — "I did this today" resets the clock; **Log it for another day** records the date it actually happened (never a future one), which is the date the clock restarts from. Once done, the button reads **Done today ✓** rather than offering itself again, and a second press on the same day doesn't record a second completion |
| **Register screen** — split by shape, with current-period state per rhythm | ✅ | ✅ | ✅ | ✅ Done |
