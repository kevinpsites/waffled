...
| Feature | Web / Kiosk | iPhone | iPad | Status |
| --- | :---: | :---: | :---: | --- |
| **Rhythms module** (`rhythms`, default **OFF** — opt-in in Settings → Modules) | ✅ | 🚧 | 🚧 | ✅ Done (web/kiosk) — gates the nav entry, the routes, the Today card and the rhythm countdown source |
| Create a rhythm — title · emoji · cadence (every N days/weeks/months/years) · assignee or whole household · anchor · notes | ✅ | 🚧 | 🚧 | ✅ Done (web/kiosk) |
...
| **Auto-schedule** a scheduling rhythm (the cadence fully determines when → one recurring event, created once) | ✅ | 🚧 | 🚧 | ✅ Done — monthly rhythms pick same-date / same-weekday / last-weekday; an advanced RRULE field is the escape hatch |
| **Nudge runway** (`leadTime`, default 14 days) — "warn me ahead" / "start nudging me before the period ends" | ✅ | 🚧 | 🚧 | ✅ Done — clamped server-side to at most **half the cadence**: a runway longer than the cycle never closes, so the item would never go quiet |
...
| **Mark a completion rhythm done** (any time, not just when due) | ✅ | 🚧 | 🚧 | ✅ Done — "I did this today" resets the clock; each completion is recorded, and the register shows **Last done** / **Next due** |
| **Register screen** — split by shape, with current-period state per rhythm | ✅ | 🚧 | 🚧 | ✅ Done (web/kiosk) |
| **Edit / pause / retire a rhythm** | ✅ | 🚧 | 🚧 | ✅ Done (web/kiosk) — edit covers title · emoji · notes · assignee · cadence · nudge runway; **Pause** stops a rhythm asking without losing anything, and **Retire** removes it while keeping its completion history. `satisfiedBy`, `startsOn`, `autoSchedule` and `rrule` are deliberately not editable: re-anchoring a live rhythm would re-interpret existing skips (keyed on `period_start`) and point bookings at periods that no longer exist, so the editor explains the anchor instead of offering to move it |
...
| **Rhythms** module — the standing intentions that should keep happening (air filter every 3 months, trash weekly, a quarterly temple visit), with a Today card, a register, period booking into real calendar events and a 🔁 marker on the events that keep one. See [Rhythms](#rhythms) above | ✅ | 🚧 | 🚧 | 🟡 Partial — web/kiosk ships the module (opt-in in Settings → Modules); the iOS surface is in progress |
