...
description: How Waffled grows — built-in toggle modules and external integrations.
---

How features get added to Waffled beyond the core app, and how self-hosters / the
...
|---|---|---|---|
| Where the code runs | *Inside* Waffled (ships in the image) | A *separate* program / container | Loaded into the running Waffled server |
| Who writes it | Contributed to Waffled's source (PR or fork) | Anyone, independently | A third party, no fork |
| Its UI | Renders **inside** Waffled (Today card, a tab) | Its **own** separate window/app (or headless) | Inside Waffled |
...
UI *inside* Waffled's own screens. It has its own UI or runs headless, and talks to Waffled
...
**Decision rule for any new feature:** *Does it need to appear inside Waffled's own
interface (a Today card, a tab) or reach into other Waffled features (meals, persons)?*
...
- **No (it can stand on its own UI or run headless) → external integration (B).** Publish
  your standalone app/script; others point it at their Waffled with an API key.
...
which matches how Waffled is already built (the API is the contract; web and iOS are
independent consumers).

## First modules (Kevin's use cases)
All three want to live *inside* Waffled (Today card / tabs / meal-plan integration), so all
