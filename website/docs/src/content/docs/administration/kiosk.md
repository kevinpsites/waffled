...
- Set the tablet's **server address** to the **Mac/host LAN IP** — that's all that's
  needed, since sync follows the address the device used. If you deliberately pinned
  `POWERSYNC_PUBLIC_URL` in `infra/compose/.env`, it must match; a stale value shows an
  **"Offline"** banner. (An address Waffled itself pinned on an older install is cleared
  automatically the next time you run `./waffled up` or `./waffled upgrade`.) See
  [Reverse proxy & TLS](/install/reverse-proxy/) and
