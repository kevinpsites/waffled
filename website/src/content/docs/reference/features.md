...
| Pantry: **running-low threshold** (household default + per-item), **per-location icons** | ✅ | 🟡 | 🟡 | ✅ Done (web); mobile: **Low** badge off the threshold + **per-location icons** in the sidebar; the household **running-low default** is now editable in **Settings → Pantry** (`PUT /api/pantry/config`); per-item threshold + per-location/icon config stay web-only |
