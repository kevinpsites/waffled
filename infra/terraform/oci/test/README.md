...
files, the `docker-compose.oci.yml` override the bootstrap writes, and a complete `.env` —
...
It then asserts the HTTPS override landed (ports 443 + 8443 published), that an `app_env`
value is injected, and negatively that **every** required secret is enforced (discovered
from the compose file, so a newly-added one is covered automatically).
...
1. **`terraform apply`** to a throwaway instance, then hit `https://<domain>` and
   `sudo waffled-oci status` (see the [module README](../README.md)). This is the truest test.
2. **Locally**, against a throwaway compose project so it can't clobber a real stack:
   clone the deployed ref, generate a full `.env` (`./waffled up` does this), then bring it
   up with the OCI override under a unique project name:
   ```bash
   docker compose -p waffled-ocitest \
     -f docker-compose.yml -f docker-compose.oci.yml --env-file .env up -d
   docker compose -p waffled-ocitest ps        # wait for healthy
   docker compose -p waffled-ocitest down -v   # clean up (deletes its volumes)
   ```
