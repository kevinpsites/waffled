...
- **"Plan my week" now works even without an AI provider — it shuffles.** Households that haven't
  configured an LLM used to get an error when they asked the planner to fill a week. Instead, the
  planner now deals a random hand from your own recipe library: it fills only the empty dinner
  slots, skips anything already planned that week or cooked in the last couple of weeks, and leaves
  slots you've already set alone. No AI required, and the app doesn't change — the same "Plan my
  week" button just always returns a plan.
